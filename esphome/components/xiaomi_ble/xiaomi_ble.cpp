...
bool parse_xiaomi_data_byte(uint8_t data_type, const uint8_t *data, uint8_t data_length, XiaomiParseResult &result) {
  switch (data_type) {
    case 0x0D: {  // temperature+humidity, 4 bytes, 16-bit signed integer (LE) each, 0.1 °C, 0.1 %
      ...
      ESP_LOGD(TAG, "  Temp: %.2f°C", temperature / 10.00f);
      ESP_LOGD(TAG, "  Hum: %.2f%%", humidity / 10.00f);
      // uint16_t output;
      //output = (uint16_t(data[0] & 0xFF) << 8) | (data[1] & 0xFF);
      ESP_LOGD(TAG,  "data0: 0x%04X", uint16_t(data[0]));
      ESP_LOGD(TAG,  "data0: 0x%08X", uint16_t(data[0]));
      ESP_LOGD(TAG,  "data1: 0x%04X", uint16_t(data[1]));
      ESP_LOGD(TAG,  "data1: 0x%08X", uint16_t(data[1]));
